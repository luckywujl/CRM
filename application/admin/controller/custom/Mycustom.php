 ...
 * @icon fa fa-circle-o
 */
class Mycustom extends Backend
{
    
    /**
     * Mycustom模型对象
     * @var \app\admin\model\custom\Mycustom
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\custom\Mycustom;
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if ($this->dataCreateField) {
                    ...
                    $credit['custom_admin'] = $this->auth->id;
                    $credit['custom_credit_type'] = 0;
                }
                ...
                try {
                    ...
                    //获得刚保存的客户ID，并将客户ID赋值给受信表的custom_id
                    $credit['custom_id'] = $this->model->custom_id;
                    $creditcustom = new custom\Creditcustom();
                    //在受信用户表中生成一条受信记录，受信类型为创建者
                    $result1 = $creditcustom->save($credit);
                    Db::commit();
        	...
        	$this->assign('auth_id',$this->auth->nickname); 
        return $this->view->fetch();
    }
    ...
    public function edit($ids = null)
    {
        ...
        $this->assign('auth_id',$this->auth->nickname); //把用户昵称传到页面
